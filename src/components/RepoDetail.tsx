...
export function RepoDetailPage({ name, initialDetail, initialSettings }: RepoDetailPageProps) {
  ...
  useEffect(() => {
    if (initialDetail) {
      setData(initialDetail)
      setSettings(initialSettings ?? null)
      setContents(initialDetail.contents || [])
      setCurrentPath('')
      setPending(false)
      setError('')
      return
  ...
  if (error || !repo) {
    return (
      <div className="text-center py-20 space-y-4">
        ...
        <Link to="/projects" className="text-sm" style={{ color: c }}>
          返回项目列表
        </Link>
      </div>
    )
        ...
        <div className="space-y-6 min-w-0">
          <div className="repo-panel">
                  ...
                  return (
                    <span key={path} className="flex items-center gap-1">
                      ...
                      {isLast ? (
                        ...
                        <button type="button" onClick={() => openDir(path)} style={{ color: c }}>
                          {part}
                        </button>
                      )}
                    </span>
            ...
            <div className={`repo-file-list ${dirPending ? 'opacity-50' : ''}`}>
              ...
              {!contents.length && (
                <div className="text-sm py-6 text-center" style={{ color: '#71717a' }}>
                  空目录
                </div>
              )}
            </div>
                  ...
                  .map(([lang, pct]) => (
                    <div key={lang} className="flex items-center justify-between text-xs">
                      <span className="flex items-center gap-2" style={{ color: '#a1a1aa' }}>
                        <span className="w-2 h-2" style={{ backgroundColor: langColor(lang) }} />
                        {lang}
                      </span>
            ...
            <div className="space-y-3">
              {data.commits?.map((commit) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="repo-commit-row block"
                >
                  <div className="flex items-center gap-2 mb-1">
                    {commit.avatar_url && (
                      <img src={commit.avatar_url} alt="" className="repo-detail-avatar w-5 h-5" />
                    )}
